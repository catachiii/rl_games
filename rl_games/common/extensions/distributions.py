...
class TwoHotDist(distr.one_hot_categorical.OneHotCategorical):
    ...
    def log_prob(self, x):
        ...
        lower_log_prob = super().log_prob(F.one_hot(lower_indices.squeeze(1), num_classes=len(self.buckets))).unsqueeze(
            1)
        upper_log_prob = super().log_prob(F.one_hot(upper_indices.squeeze(1), num_classes=len(self.buckets))).unsqueeze(
            1)
